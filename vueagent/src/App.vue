<template>
  <div class="terminal-page">
    <div class="button-container">
      <button @click="sendCommand('cd /root && chmod +x agent.sh && ./agent.sh')" class="cmd-button">安装与升级</button>
      <button v-if="screens.length==0" @click="sendCommand('cd /root/bot && screen -S bot')" class="cmd-button">创建屏幕</button>
      <div v-for="scr in screens" :key="scr" class="screen-container">
        <button @click="sendCommand('cd /root/bot && screen -r ' + scr)" class="cmd-button">
          屏幕{{ scr }}
        </button>
        <span @click="closeScreen(scr)" class="close-button">×</span>
      </div>
      <button @click="sendCommand('cd /root/bot && ./run.sh')" class="cmd-button">运行程序</button>
      <button @click="sendCommand('Ctrl+C')" class="cmd-button">终止运行</button>
      <button @click="sendCommand('cd /root/bot && ./kill-process.sh')" class="cmd-button">强制停止</button>
      ...
      <button @click="uppage()" class="cmd-button">刷新页面</button>
    </div>
...
const closeScreen = async (scr:string) => {
  ...
  if (socket.value?.readyState === WebSocket.OPEN && terminal.value) {
    var command = 'cd /root/bot && screen -X -S '+scr+' quit'
...
const sendCommand = (command: string) => {
  ...
  if (socket.value?.readyState === WebSocket.OPEN && terminal.value) {
    terminal.value.writeln('\n' + command);
    socket.value.send(command + "\n");
    if(command=='cd /root/bot && screen -S bot'){
      setTimeout(() => {
        fetchScreen();
      }, 5000); // 5000 毫秒 = 5 秒
    }
...
const fetchScreen = async () => {
  try {
    const response = await fetch(hosturl+'/screen');
    if (response.ok) {
      const content = await response.json();
      if(content.error){
...
function connectWebSocket() {
  ...
  const socketUrl = hosturl+'/ws' + urlmodel;
